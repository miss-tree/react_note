    ...
    constructor(props){
        ...
        this.state={
            msg:[
                ...
                {name:"i",explain:"在进入编辑界面后 按下 i 键进入输入模式"},
                {name:":wq",explain:"保存修改(编辑好后按ESC退出编辑，然后输入)"},
                {name:":q!",explain:"放弃修改(按ESC退出编辑，然后输入)"},
                ...
                {name:"find  /etc/nginx  nginx.conf",explain:"查找/etc/nginx下的nginx.conf"},
                {name:"find /etc -iname init",explain:"查找/etc/下的init,不区分大小写"},
                {name:"find /etc -name *init",explain:"查找/etc/下的init,以 init 结尾的文件或目录名"},
                {name:"find /etc -name init???",explain:"查找/etc/下的init，？ 表示单个字符，即搜索到 init___"},
